...
start=`date +%s.%N`
php ServiceInsercaoClaroMariaDB.php
end=`date +%s.%N`
...
start=`date +%s.%N`
php ServiceBuscarNotificacoesClaroMariaDB.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceAlterarEstadoClaroMariaDB.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceDeletarNotificacoesClaroMariaDB.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
