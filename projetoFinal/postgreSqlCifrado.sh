...
start=`date +%s.%N`
php ServiceInsercaoCifradaPostgreSql.php
end=`date +%s.%N`
...
start=`date +%s.%N`
php ServiceBuscarNotificacoesCifradasPostgreSql.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceAlterarEstadoCifradoPostgreSql.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceDeletarNotificacoesCifradasPostgreSql.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
