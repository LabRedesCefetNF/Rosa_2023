...
cd src/service/mariadb/
start=`date +%s.%N`
php ServiceInsercaoCifradaMariaDB.php
end=`date +%s.%N`
...
start=`date +%s.%N`
php ServiceBuscarNotificacoesCifradasMariaDB.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceAlterarEstadoCifradoMariaDB.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceDeletarNotificacoesCifradasMariaDB.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
