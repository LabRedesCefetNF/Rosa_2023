...
cd src/service/firebird/
start=`date +%s.%N`
php ServiceInsercaoNotificacaoClaroFirebird.php
end=`date +%s.%N`
...
start=`date +%s.%N`
php ServiceBuscarNotificacoesClaroFirebird.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceAlterarEstadoClaroFirebird.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceDeletarNotificacoesClaroFirebird.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
