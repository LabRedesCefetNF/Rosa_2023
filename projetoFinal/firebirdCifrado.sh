...
start=`date +%s.%N`
php ServiceInsercaoCifradaFirebird.php
end=`date +%s.%N`
...
start=`date +%s.%N`
php ServiceBuscarNotificacoesCifradasFirebird.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceAlterarEstadoCifradoFirebird.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
...
start=`date +%s.%N`
php ServiceDeletarNotificacoesCifradasFirebird.php
end=`date +%s.%N`
runtime=$( echo "$end - $start" | bc -l )
# parando o monitor de consumo
kill $pid
